...
def EM1(x, y):
    ...
    # c_1 finner vi ved å dele r_0 på r_1 og runde ned til nærmeste heltall. Dette kan vi gjøre med numpy.floor funksjonen
    ...
    # Første [-1] sier at vi ser på siste likning i likninger, andre [-1] sier at vi ser på siste entry i likningen
    while likninger[-1][-1] != np.gcd(x, y):
...
def Losning(a, b):
    ...
    for tuppel in matrise:
        print(str(tuppel[0]) + " = " + str(tuppel[1]) +
              " · " + str(tuppel[2]) + " + " + str(tuppel[3]))
        print(" ")
    ...
    reversering = [[matrise[-1][-1], 1, matrise[-1]
                    [0], -matrise[-1][1], matrise[-1][2]]]
